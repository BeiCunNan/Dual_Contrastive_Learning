...
def get_config():
    ...
    num_classes = {'sst2': 2, 'subj': 2, 'trec': 6, 'pc': 2, 'cr': 2,'sst5':5}

    ''' Base '''
    parser.add_argument('--data_dir', type=str, default='data')
    parser.add_argument('--dataset', type=str, default='', choices=num_classes.keys())
    parser.add_argument('--model_name', type=str, default='bert', choices=['bert', 'roberta'])
    parser.add_argument('--method', type=str, default='nl2b', choices=['ce', 'scl', 'dualcl','nl','nl1a','nl1b','nl2a','nl2b','nl3a','nl3b','pos']) # CE为交叉熵、SCL为标准对比学习
    ''' Optimization '''
    ...
    ''' Environment '''
    parser.add_argument('--backend', default=False, action='store_true') # 触发工具,若使用了backend,则python config.py返回值为true,反之为false
    parser.add_argument('--timestamp', type=int, default='{:.0f}{:03}'.format(time.time(), random.randint(0, 999)))
    parser.add_argument('--device', type=str, default='cpu' ) #    parser.add_argument('--device', type=str, default='cuda' )
